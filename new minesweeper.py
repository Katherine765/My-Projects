...
def click(event):
    ...
    if opened == 0:
        ...
        if loc in grid:
            ...
            clicked[loc]=canvas.create_text(x, y, text=grid[loc], font = 'Helvetica 20')
            opened += 1
            if grid[loc] == 0:
...
def check_for_win():
    if opened == WIDTH*HEIGHT-NUM_MINES+1: #the plus one is because the first click adds an additional open so that the start function only runs once
